  ...
  public function build() {
    ...
    foreach ($get_all_months_name as $key => $month_name) {
      if($key<=$current_month){
	      ...
	      $user_count += count(getAuditOPtions('auditor_selection','/auditor-and-audit-export/'.$uri[1].'?created[min]='.$first_last_date_monthly[$month_name]['first_day'].'&created[max]='.$first_last_date_monthly[$month_name]['last_day']));
